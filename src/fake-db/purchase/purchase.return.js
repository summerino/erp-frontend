...
mock.onGet(`/api/${endpoint.purchase.return}`).reply(async (config) => {
  var search = null
  if (config.params) {
  ...
  const resp_h = await axiosJsonServer.get(`/purchaseReturn_H?q=${search}`)
...
mock.onGet(`/api/${endpoint.purchase.return}/item`).reply(async (config) => {
  const { code } = config.params
  
  const resp_d = await axiosJsonServer.get(`/purchaseReturn_D?code=${code}&_sort=lineNo`)
...
mock.onGet(/\/api\/purchase-receive\/uninv\/./).reply(async (config) => {
  ...
  const resp_h = await axiosJsonServer.get(`/purchaseReturn_H?supCode=${urlSegment[urlSegment.length - 1]}`)
...
mock.onPost(`/api/${endpoint.purchase.return}`).reply(async (request) => {
  const data = JSON.parse(request.data)
  
  const response_h = await axiosJsonServer.get('/purchaseReturn_H')
  ...
  axiosJsonServer.post('/purchaseReturn_H', {
    code: code,
    refNo: data.refNo,
  ...
  for (var i = 0; i < data.itemDetails.length; i++) {
    axiosJsonServer.post('/purchaseReturn_D', {
...
mock.onPut(/\/api\/purchase-receive\/./).reply(async (config) => {
  ...
  const response_h = await axiosJsonServer.get(`/purchaseReturn_H?code=${data.code}`)
  const id = response_h.data[0].id

  // Update purchase receive header
  axiosJsonServer.put(`/purchaseReturn_H/${id}`, {
    code: data.code,
    refNo: data.refNo,
    receiveDate: data.receiveDate,
    poCode: data.poCode,
  ...
  const response_d = await axiosJsonServer.get(`/purchaseReturn_D?code=${data.code}`)
  const delItem = response_d.data.filter(d => !data.itemDetails.map(i => i.id).includes(d.id))
  
  for (var i = 0; i < delItem.length; i++) {
    axiosJsonServer.delete(`/purchaseReturn_D/${delItem[i].id}`)
  }

  // Update purchase receive details
  for (var i = 0; i < data.itemDetails.length; i++) {
    if (data.itemDetails[i].id) {
      axiosJsonServer.put(`/purchaseReturn_D/${data.itemDetails[i].id}`, {
        rowId: data.itemDetails[i].rowId,
...
mock.onDelete(/\/api\/purchase-receive\/./).reply(async (config) => {
  ...
  const response_h = await axiosJsonServer.get(`/purchaseReturn_H?code=${urlSegment[urlSegment.length - 1]}`)
  if (response_h.data.length > 0) {
    axiosJsonServer.delete(`/purchaseReturn_H/${response_h.data[0].id}`)
  }

  // Delete purchase receive details
  const response_d = await axiosJsonServer.get(`/purchaseReturn_D?code=${urlSegment[urlSegment.length - 1]}`)
  for (var i = 0; i < response_d.data.length; i++) {
    axiosJsonServer.delete(`/purchaseReturn_D/${response_d.data[i].id}`)
  }
