    ...
    <v-card>
      <v-card-title class="indigo--text text--lighten-2 pb-1">
        <v-row dense>
          ...
          <v-spacer></v-spacer>
          <v-col cols="12" md="1">
            <export-excel title="Daftar Peran" :grid="grid" :gridDefOpts="gridDefOpts" :filters="filter" ref="exportExcel"></export-excel>
          </v-col>
...
export default {
  ...
  created: function () {
    this.getList()
    this.loadAction()
    auth.getAction(this.endpoint, this.menuId.role, [this.action.insert, this.action.update, this.action.delete])
      .then((response) => {
