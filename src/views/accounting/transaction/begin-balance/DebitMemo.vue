...
export default {
  ...
  methods:{
    ...
    getList(bindToForm = false) {
      ...
      api.getAll(this.endpoint.accounting.debitMemo, {
        params: {
    ...
    async remove(item) {
      if (
        ...
        api.deleteData(this.endpoint.accounting.debitMemo, item)
          .then(response => {
    ...
    async save() {
      ...
      if (this.data.action === 'add') {
        const resp = await api.create(this.endpoint.accounting.debitMemo, this.data)
        result = resp.data
      } else if (this.data.action === 'edit') {
        const resp = await api.update(this.endpoint.accounting.debitMemo, this.data.id, this.data)
        result = resp.data
      }
