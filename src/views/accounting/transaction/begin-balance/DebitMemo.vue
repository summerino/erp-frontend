  ...
  <div class="w-full">
    <v-card v-if="main">
      <v-card-title class="indigo--text text--lighten-2 pb-1">
          ...
          <v-col cols="12" md="5">
            <v-row no-gutters>
              ...
              <export-excel
                ...
                title="Daftar Saldo Awal Nota Debit"
              ></export-excel>
            </v-row>
          </v-col>
          <v-col cols="12" md="1" class="text-right">
            <template-excel title="Template Saldo Awal Nota Debit" :grid="grid"></template-excel>
          </v-col>
          <v-col cols="12" md="1" class="text-right">
            <v-tooltip bottom>
              <template v-slot:activator="{ on, attrs }">
              ...
              <span class="text-caption text-center">Import Data Saldo Awal Nota Debit<br/>(Ctrl + Alt + U)</span>
            </v-tooltip>
          </v-col>
        ...
        <v-row v-else no-gutters>
          <v-col cols="12" md="4">
            <span>Import Saldo Awal Nota Debit</span>
          </v-col>
    ...
    <v-card v-else>
      <v-card-title class="indigo--text text--lighten-2 pb-1">
        <v-row dense>
          <v-col cols="12" md="4">
            <span>{{ data.action === 'add' ? 'Tambah' : 'Ubah' }} Saldo Awal Nota Debit</span>
          </v-col>
          <v-col cols="12" md="8" class="text-right">
            <label
    ...
    <upload-excel ref="upload" type="bb-dm" :grid="grid" @uploadCmp="changePosting"></upload-excel>
  </div>
</template>
...
export default {
  ...
  methods:{
    ...
    getList(bindToForm = false, filters = []) {
      ...
      api.getAll(this.endpoint.accounting.beginBalance.dm, {
        params: {
    ...
    async remove(item) {
      if (
        ...
        api.delete(this.endpoint.accounting.beginBalance.dm, item.id, {data: item})
          .then(response => {
    ...
    async save() {
      ...
      if (this.data.action === 'add') {
        const resp = await api.create(this.endpoint.accounting.beginBalance.dm, this.data)
        result = resp.data
      } else if (this.data.action === 'edit') {
        const resp = await api.update(this.endpoint.accounting.beginBalance.dm, this.data.id, this.data)
        result = resp.data
      }
    ...
    async postData() {
      ...
      let result = { success: false, message: '' }
      const resp = await api.create(`${this.endpoint.accounting.beginBalance.dm}/posting`, data)
      result = resp.data
