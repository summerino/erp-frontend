  ...
  <div class="w-full">
          ...
          <v-col cols="12" md="5">
            <v-row no-gutters>
              ...
              <export-excel
                ...
                title="Daftar Saldo Awal Piutang"
              ></export-excel>
            </v-row>
          </v-col>
    ...
    <v-card v-else>
      <v-card-title class="indigo--text text--lighten-2 pb-1">
        <v-row dense>
          <v-col cols="12" md="4">
            <span>{{ data.action === 'add' ? 'Tambah' : 'Ubah' }} Saldo Awal Piutang</span>
          </v-col>
          <v-col cols="12" md="8" class="text-right">
            <label
...
export default {
  ...
  created: function () {
    ...
    auth.getAction(this.endpoint, this.menuId.bbAccountReceivable)
      .then((response) => {
  ...
  methods:{
    ...
    getList(bindToForm = false, filters = []) {
      ...
      api.getAll(this.endpoint.accounting.beginBalance.ar, {
        params: {
    ...
    async remove(item) {
      if (
        ...
        api.delete(this.endpoint.accounting.beginBalance.ar, item.id, {data: item})
          .then(response => {
    ...
    async save() {
      ...
      if (this.data.action === 'add') {
        const resp = await api.create(this.endpoint.accounting.beginBalance.ar, this.data)
        result = resp.data
      } else if (this.data.action === 'edit') {
        const resp = await api.update(this.endpoint.accounting.beginBalance.ar, this.data.id, this.data)
        result = resp.data
      }
