  ...
  <div class="w-full">
    ...
    <v-card v-else>
      <v-card-title class="indigo--text text--lighten-2 pb-1">
        <v-row dense>
          <v-col cols="12" md="4">
            <span>{{ data.action === 'add' ? 'Tambah' : 'Ubah' }} Piutang</span>
          </v-col>
...
export default {
  ...
  methods:{
    ...
    getList(bindToForm = false) {
      ...
      api.getAll(this.endpoint.accounting.accountReceivable, {
        params: {
    ...
    async remove(item) {
      if (
        ...
        api.deleteData(this.endpoint.accounting.accountReceivable, item)
          .then(response => {
    ...
    async save() {
      ...
      if (this.data.action === 'add') {
        const resp = await api.create(this.endpoint.accounting.accountReceivable, this.data)
        result = resp.data
      } else if (this.data.action === 'edit') {
        const resp = await api.update(this.endpoint.accounting.accountReceivable, this.data.id, this.data)
        result = resp.data
      }
