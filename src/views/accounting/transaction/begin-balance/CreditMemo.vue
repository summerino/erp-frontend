<template>
  <div class="w-full">
    <v-card v-if="main">
      <v-card-title class="indigo--text text--lighten-2 pb-1">
        <v-row v-if="!posting" no-gutters>
          ...
          <v-col cols="12" md="5">
            <v-row no-gutters>
              ...
              <export-excel title="Daftar Saldo Awal Nota Kredit" :grid="grid" :gridDefOpts="gridDefOpts" :filters="filter" ref="exportExcel"></export-excel>
            </v-row>
          </v-col>
          <v-col cols="12" md="1" class="text-right">
            <template-excel title="Template Saldo Awal Nota Kredit" :grid="grid"></template-excel>
          </v-col>
          <v-col cols="12" md="1" class="text-right">
            <v-tooltip bottom>
              <template v-slot:activator="{ on, attrs }">
              ...
              <span class="text-caption text-center">Import Data Saldo Awal Nota Kredit<br/>(Ctrl + Alt + U)</span>
            </v-tooltip>
          </v-col>
        ...
        <v-row v-else no-gutters>
          <v-col cols="12" md="4">
            <span>Import Saldo Awal Nota Kredit</span>
          </v-col>
    ...
    <upload-excel ref="upload" type="bb-cm" :grid="grid" @uploadCmp="changePosting"></upload-excel>
...
export default {
  ...
  created: function () {
    ...
    auth.getAction(this.endpoint, this.menuId.bbCreditMemo)
      .then((response) => {
  ...
  methods:{
    ...
    async postData() {
      ...
      let result = { success: false, message: '' }
      const resp = await api.create(`${this.endpoint.accounting.beginBalance.cm}/posting`, data)
      result = resp.data
