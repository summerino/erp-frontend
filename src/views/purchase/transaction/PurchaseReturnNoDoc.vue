            ...
            <v-tooltip bottom>
              <template v-slot:activator="{ on, attrs }">
                <v-btn
                  ...
                  :disabled="isVoid || hasRelatedTrans || (data.action === 'edit' && (!auth.allowUpdate || seenByOthers)) || isClosed"
                  dark
                  text
                  @click="save(true)"
                  @shortkey="save(true)"
                        ...
                        >
                          ...
                          <template v-slot:[`item.warehouseInitial`]="{ item }">
                            <v-autocomplete
                              ...
                              :disabled="hasRelatedTrans || isClosed"
                              :items="warehouses"
                              :rules="rules.required"
                              item-text="initial"
                              item-value="code"
...
export default {
  ...
  methods: {
    ...
    calcPrice() {
      ...
      this.data.exemptTaxAmountOut = _sumBy(this.gridItem.data, 'totExemptTax')
      ...
      this.data.exemptTaxAmountIn = _sumBy(this.gridDiffItem.data, 'totExemptTax')
      this.data.dppIn = _sumBy(this.gridDiffItem.data, 'totDPP')
      
      this.data.taxAmount = this.data.taxAmountIn + this.data.taxAmountOut
      this.data.exemptTaxAmount = this.data.exemptTaxAmountIn + this.data.exemptTaxAmountOut
    ...
    calcGrandTotal() {
      ...
      this.data.total = this.data.totalOut
    },
    showFindSupDialog() {
