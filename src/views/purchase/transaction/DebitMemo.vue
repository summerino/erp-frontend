<template>
  <div class="w-full">
    ...
    <memo-to-cashbank ref="cashBank" :memo="data" source="debit-memo" :transactionType="transactionType" @closeParent="close"></memo-to-cashbank>
...
export default {
  ...
  data: () => ({
    ...
    grid: {
      columns: [
        ...
        { text: 'Pemasok', value: 'supName', divider: true, width: '200', excelColWidth:'35' },
  ...
  created: function () {
    ...
    auth.getAction(this.endpoint, this.menuId.debitMemo)
      .then((response) => {
    ...
    auth.getAction(this.endpoint, this.menuId.cashBank, [1])
      .then((response) => {
        this.allowInsertCashBank = response.data.some(x => x)
      })
