                ...
                <v-card>
                  ...
                  <v-card-text>
                    ...
                    <v-row no-gutters>
                      <v-col cols="12">
                        <v-text-field
                          v-model.trim="data.rcvCode"
                          :readonly="hasRelatedTrans"
                          :rules="rules.required"
                          label="Rcv. Code"
                          class="mt-0"
                          required
                          @change="rcvCodeChange"
                        >
...
import FindTransItem from '@/components/dialog/FindTransItem'
  ...
  methods: {
    ...
    getRcvItemLists() {
      api.getAll(`${this.endpoint.purchase.receive}/item`, {
        ...
        .then(response => {
          this.rcvDetails = response.data.tableData
        })
    },
    getTaxLists() {
    ...
    addItem() {
      if (!this.data.rcvCode) {
        this.$store.dispatch('app/showInfo', 'Please input purchase receive first.')
        return
      }
    ...
    itemIdChange(item) {
      ...
      if (rcvDetail) {
        ...
        item.oldUnitPrice = rcvDetail.oldUnitPrice
        item.unitId = rcvDetail.unitId
        item.unitName = rcvDetail.unitName
        item.unitPrice = rcvDetail.unitPrice
        item.disc = 0
        item.taxId = rcvDetail.taxId
        item.taxAmount = 0
        item.nettPrice = rcvDetail.unitPrice
        item.dpp = rcvDetail.unitPrice
        if (item.state !== 'A') {
          item.state = 'M'
        }
    ...
    showFindRcvDialog() {
      this.$refs.findRcv.open()
    },
    showFindSupDialog() {
      this.$refs.findSup.open()
    },
    showFindItemDialog(item) {
      this.$refs.FindTransItem.open(item)
    },
    bindRcvData(rowItem, item) {
      if (item) {
        this.data.rcvCode = item.code
