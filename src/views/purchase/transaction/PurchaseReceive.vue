                            ...
                            <v-tooltip bottom>
                              <template v-slot:activator="{ on, attrs }">
                                <v-btn
                                  v-bind="attrs"
                                  v-on="on"
                                  :disabled="item.type == 0 || isVoid || hasRelatedTrans || (!auth.allowInsert && (data.action === 'edit' && !auth.allowUpdate))"
                                  color="red"
                                  icon
                                  small
...
export default {
  ...
  created: function () {
    ...
    auth.getAction(this.endpoint, this.menuId.purchasereceive, [this.action.insert, this.action.update, this.action.void, this.action.changeDate])
      .then((response) => {
