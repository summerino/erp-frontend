                          ...
                          <v-tooltip bottom>
                            <template v-slot:activator="{ on, attrs }">
                              <v-btn
                                ...
                                v-on="on"
                                v-shortkey="['ctrl', 'i']"
                                :disabled="isVoid || isPurchaseReturn || hasRelatedTrans || (data.action === 'add' && !auth.allowCreate) || (data.action === 'edit' && !auth.allowUpdate)"
                                class="blue--text"
                                small
                                tile
                                @click="addItem"
...
export default {
  ...
  methods: {
    ...
    async close() {
      if (this.data.action === 'edit') {
        activeTrans.released('RCV', this.data.code)
      }
    ...
    async edit(item) {
      ...
      const resp = await activeTrans.locked('RCV', item.code)
    ...
    async remove(item) {
      if (
        ...
        api.delete(this.endpoint.purchase.receive, item.code, {data: item})
          .then(response => {
          ...
          api.getAll(item.type === 2 ? `${this.endpoint.purchase.return}/item` : `${this.endpoint.purchase.return}/diff-item`, {
            ...
            .then(response => {
              ...
              for (let i = 0; i < this.gridItem.data.length; i++) {
                ...
                this.gridItem.data[i].outstandingQty = this.gridItem.data[i].qty - this.gridItem.data[i].qtyRcv
                this.gridItem.data[i].qty = this.gridItem.data[i].outstandingQty
                this.gridItem.data[i].warehouseCode = item.type === 2 ? this.gridItem.data[i].warehouseCodeIn : this.gridItem.data[i].warehouseCode
