        ...
        <v-card>
          <v-card-title class="indigo--text text--lighten-2 pb-1">
                    ...
                    <v-list-item>
                      <v-list-item-title>
                        <export-excel
                          ...
                          title="Daftar Laporan Retur Penjualan"
                        ></export-excel>
                      </v-list-item-title>
                    </v-list-item>
                  </v-list>
            ...
            <v-row v-else no-gutters>
              <v-col cols="12" md="8">
                Laporan Retur Penjualan - Detail {{ this.data.typeName }} - {{ this.data.detailName}}
              </v-col>
                    ...
                    <v-list-item>
                      <v-list-item-title>
                        <export-excel
                          ...
                          :grid="grid"
                          :gridDefOpts="gridDefOpts"
                          :title="`Daftar Laporan Retur Penjualan - Detail ${ this.data.typeName } - ${ this.data.detailName }`"
                        ></export-excel>
                      </v-list-item-title>
                    </v-list-item>
                  </v-list>
...
export default {
  ...
  data: () => ({
    ...
    codeColumn: [
      ...
      { text: 'Tipe', value: 'typeName', divider: true, width: '160', excelColWidth:'20' },
    ...
    detailColumn: [
      ...
      { text: 'Gudang Masuk', value: 'warehouseIn', divider: true, width: '160', excelColWidth:'20' },
      { text: 'Ins. Kategori', value: 'categoryInitial', divider: true, width: '120', excelColWidth:'12' },
      { text: 'Qty', value: 'qty', align: 'right', divider: true, width: '100', excelColWidth:'20' },
      { text: 'Satuan', value: 'unitName', divider: true, width: '120', excelColWidth:'12' },
    ...
    itemCategories: [],
    items: [],
    statuses: [{ id: 'A', name: 'Aktif' }, { id: 'PS', name: 'Dikirim Sebagian' },
      { id: 'CMP', name: 'Dikirim Seluruhnya' }, { id: 'V', name: 'Void' }, { id: 'NV', name: 'Bukan Void' }],
    customers: [],
  ...
  created: function () {
    ...
    auth.getAction(this.endpoint, this.menuId.salesReturnReport)
      .then((response) => {
  ...
  methods:{
    ...
    getList() {
      ...
      api.getAll(this.endpoint.sales.srReport, {
        params: {
          type: this.data.type,
    ...
    dblclickRow(event, { item }) {
      if (this.main && this.data.type < 5) {
        ...
        if (this.data.type === 1) {
          ...
          const cust = this.customers.find(x => x.code === item.code)
          this.data.detailName = `${cust.initial} - ${cust.name} (${cust.code})`
