...
export default {
  ...
  created: function () {
    ...
    this.getTaxLists()
    this.getItemLists()
    auth.getAction(this.endpoint, this.menuId.salesreturn, [this.action.insert, this.action.update, this.action.void, this.action.changeWarehouse, this.action.changeDate])
      .then((response) => {
  ...
  methods: {
    ...
    async remove(item) {
      if (
        ...
        api.deleteData(this.endpoint.sales.return, item)
          .then(response => {
