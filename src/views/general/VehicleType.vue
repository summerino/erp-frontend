  ...
  <div class="w-full">
    <v-card v-if="main">
      <v-card-title class="indigo--text text--lighten-2 pb-1">
        <v-row dense>
          ...
          <v-spacer></v-spacer>
          <v-col cols="12" md="1">
            <export-excel title="Daftar Jenis Kendaraan" :grid="grid" :gridDefOpts="gridDefOpts" :filters="filter" ref="exportExcel"></export-excel>
          </v-col>
    ...
    <v-card v-else>
      <v-card-title class="indigo--text text--lighten-2 pb-1">
        <v-row dense>
          <v-col cols="12" md="6">
            <span>{{ data.action === 'add' ? 'Tambah' : 'Ubah' }} Tipe Kendaraan</span>
          </v-col>
...
export default {
  ...
  created: function () {
    this.getList()
    auth.getAction(this.endpoint, this.menuId.vehicletype, [this.action.insert, this.action.update, this.action.delete])
      .then((response) => {
