          ...
          <v-col cols="12" md="6">
            <v-row no-gutters>
              ...
              <export-excel
                ...
                title="Daftar Transaksi Yang Aktif"
              ></export-excel>
            </v-row>
          </v-col>
...
export default {
  ...
  data: () => ({
    filterFields: [{
      ...
      text: 'Sumber Transaksi', value: 'srcName', dataType: 'text'
    }],
    grid: {
      columns: [
  ...
  created: function () {
    this.getList()
    auth.getAction(this.endpoint, this.menuId.activeTransaction)
      .then((response) => {
  ...
  methods:{
    ...
    getList(filters = []) {
      ...
      api.getAll(this.endpoint.general.activeTransaction, {
        params: {
    ...
    async save() {
      ...
      const resp = await api.create(this.endpoint.general.activeTransaction, this.selected)
      result = resp.data
      if (result.success) {
