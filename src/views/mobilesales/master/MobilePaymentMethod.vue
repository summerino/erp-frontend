  ...
  <div class="w-full">
    ...
    <v-card v-else>
      <v-card-title class="indigo--text text--lighten-2 pb-1">
        <v-row dense>
          <v-col cols="12" md="4">
            <span>{{ data.action === 'add' ? 'Tambah' : 'Ubah' }} Metode Pembayaran</span>
          </v-col>
...
export default {
  ...
  data: () => ({
    ...
    grid: {
      columns: [
        ...
        { text: 'Kode Akun', value: 'coaCode', divider: true, width: '150', excelColWidth:'20' },
        { text: 'Nama Akun', value: 'coaName', divider: true, width: '150', excelColWidth:'20' },
  ...
  created: function () {
    this.getList()
    this.getCOAList()
    auth.getAction(this.endpoint, this.menuId.mobilePaymentMethod)
      .then((response) => {
  ...
  methods:{
    ...
    getList(bindToForm = false) {
      ...
      api.getAll(this.endpoint.mobileSales.paymentMethod, {
        params: {
    ...
    async remove(item) {
      if (
        ...
        api.delete(this.endpoint.mobileSales.paymentMethod, item.id)
          .then(response => {
    ...
    async save() {
      ...
      if (this.data.action === 'add') {
        const resp = await api.create(this.endpoint.mobileSales.paymentMethod, this.data)
        result = resp.data
      } else if (this.data.action === 'edit') {
        const resp = await api.update(this.endpoint.mobileSales.paymentMethod, this.data.id, this.data)
        result = resp.data
      }
